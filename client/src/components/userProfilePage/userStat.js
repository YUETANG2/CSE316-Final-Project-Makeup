...
export default function UserStat(props) {
  ...
  useEffect(() => {
    let getUserInfos = async () => {
      let res1 = await axios.get("http://127.0.0.1:8000/profile/getUsername/" + props.userId);
      let name = res1.data;
      setUserName(name);
      console.log(userName);

      let res2 = await axios.get("http://127.0.0.1:8000/profile/getReputation/" + props.userId);
      let reputationPoints = res2.data;
      setReputation(reputationPoints);
      console.log(reputation);

      let res3 = await axios.get("http://127.0.0.1:8000/profile/getDateJoined/" + props.userId);
      let date = res3.data; 
      setTimeJoined(date);
    };
  ...
  return (
    <>
      ...
      <div className="content-div" id="filters2">
        <form id="filters-form">
          <FilterBtn
            value="Questions "
            onClick={props.findAllQstnsCreated}
            key="Questions"
          />
          <FilterBtn
            value="Answers"
            onClick={props.findAllAnsCreated}
            key="Answers"
          />
          <FilterBtn
            value="Tags"
            onClick={props.findAllTagsCreated}
            key="Tags"
          />
        </form>
      </div>
